...
function insertVotingSection(tweetElement) {
    ...
    tweetElement.querySelector(".AIBtn").addEventListener("click", function () {
        let aiCheckButton = this;
        let firstRow = tweetElement.querySelector("#FirstRow");
    
        // 🔹 Completely hide the AI Check button
        aiCheckButton.style.display = "none"; 
        aiCheckButton.style.visibility = "hidden";  // Prevents flickering due to reflow
    
        // 🔹 Add loading indicator
        let loadingIndicator = document.createElement("p");
        loadingIndicator.classList.add("LoadingText");
        loadingIndicator.textContent = "Analyzing...";
        firstRow.appendChild(loadingIndicator);
    
        // Prevent other functions from re-displaying the button
        aiCheckButton.setAttribute("data-locked", "true");
        ...
        .then(predictData => {
            ...
            // 🔹 Show voting buttons (AI Check button remains hidden!)
        ...
        .catch(error => {
            ...
            // 🔹 Remove loading text and restore AI button only if there's an error
            loadingIndicator.remove();
            aiCheckButton.style.display = "inline-block";
            aiCheckButton.style.visibility = "visible"; // Re-enable visibility
            aiCheckButton.removeAttribute("data-locked"); // Allow normal behavior again
        });
    
        // 🔹 Force-hide AI button periodically to prevent blinking
        let hideInterval = setInterval(() => {
            if (aiCheckButton.getAttribute("data-locked") === "true") {
                aiCheckButton.style.display = "none";
                aiCheckButton.style.visibility = "hidden";
            }
        }, 50);
    
        // Stop forcing hide after AI analysis (ensures cleanup)
        setTimeout(() => {
            clearInterval(hideInterval);
            aiCheckButton.removeAttribute("data-locked"); // Unlock button for future interactions
        }, 30000);
    });
